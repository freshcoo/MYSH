...
def split_argument(command_string: str) -> list[str]:
    splitted = shlex.shlex(command_string, posix=True)
    ...
    try:
        ...
        if "No closing quotation" in str(e):
            print("mysh: syntax error: unterminated quote", file=sys.stderr, flush=True)
        else:
            print(f"mysh: syntax error: {str(e)}", file=sys.stderr, flush=True)
        return []
